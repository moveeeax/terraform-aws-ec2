...
```hcl
module "ec2_instance" {
  # Using GitHub directly (recommended for development/private repos)
...
### Alternative: Using Terraform Registry (for published modules)
```hcl
module "ec2_instance" {
...
## Using GitHub as Module Source

You can use GitHub directly as a source for your Terraform modules instead of the Terraform Registry. Here are the different ways to do this:

### 1. Direct GitHub URL (Recommended for development)
```hcl
module "ec2_instance" {
  source = "github.com/moveeeax/terraform-aws-ec2"
...
### 2. GitHub URL with specific branch or tag
```hcl
module "ec2_instance" {
  source = "git::https://github.com/moveeeax/terraform-aws-ec2.git?ref=v1.0.0"
...
### 3. GitHub URL with specific commit hash
```hcl
module "ec2_instance" {
  source = "git::https://github.com/moveeeax/terraform-aws-ec2.git?ref=abc123def456"
...
### 4. Using SSH (requires SSH key setup)
```hcl
module "ec2_instance" {
  source = "git@github.com:moveeeax/terraform-aws-ec2.git"
...
```hcl
module "ec2_instance" {
  source = "git::https://github.com/moveeeax/private-terraform-module.git"
...
### 6. Local Development (for testing changes)
When developing or testing changes to this module, you can use a local path:

```hcl
module "ec2_instance" {
  source = "../terraform-aws-ec2"  # Relative path to the module
...
**Note:** When using private repositories, you'll need to configure Git credentials or use Terraform Cloud/Enterprise with VCS integration.

## Examples

- [Complete EC2 instance](https://github.com/moveeeax/terraform-aws-ec2/tree/master/examples/complete)
- [EC2 instance w/ private network access via Session Manager](https://github.com/moveeeax/terraform-aws-ec2/tree/master/examples/session-manager)
...
- `network_interface` can't be specified together with `vpc_security_group_ids`, `associate_public_ip_address`, `subnet_id`. See [complete example](https://github.com/moveeeax/terraform-aws-ec2/tree/master/examples/complete) for details.
...
Module is maintained by the [moveeeax organization](https://github.com/moveeeax) with help from [these awesome contributors](https://github.com/moveeeax/terraform-aws-ec2/graphs/contributors).

## License

MIT Licensed. See [LICENSE](https://github.com/moveeeax/terraform-aws-ec2/tree/master/LICENSE) for full details.
